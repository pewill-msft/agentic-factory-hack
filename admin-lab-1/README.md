...
This lab covers two essential model lifecycle activities: **managing model versions and upgrades** (Part A) and **fine-tuning a model** for the Contoso Tires manufacturing domain (Part B).
...
Azure AI Foundry deploys specific **versions** of a model. Over time, model providers release new versions with improvements, and older versions reach **deprecation** and eventually **retirement**. As an admin, you need to understand:
...
| **Deprecation date** | After this date, the version still works but is no longer recommended |
| **Retirement date** | After this date, the version stops working — you must upgrade before this |
...
## Part A — Model Versioning & Upgrades

### Task 1: Inspect Deployed Model Versions
...
### Task 2: Explore Model Update Policies
...
### Task 3: Upgrade a Model Deployment
...
### Task 4: Verify in the Playground
...
### Task 5: Review the Training Data
...
1. Open the training data file: `admin-lab-1/data/training-data.jsonl`
...
### Task 6: Create a Fine-Tuning Job
...
| **Training data** | Upload `admin-lab-1/data/training-data.jsonl` |
...
> [!NOTE]
> Fine-tuning typically takes **30–60 minutes** depending on the data size and selected hyperparameters. **Continue to [Admin Lab 2](../admin-lab-2/README.md) while the job runs** — you can return to check the results later.

### Task 7: Monitor Training Progress
...
6. Send the **same prompt** you used in Task 4:

> What are the most common causes of excessive drum vibration in a tire building machine, and what maintenance steps should be taken?

7. Compare the response with the base model response from Task 4.
